...
impl BotApp {
    ...
    pub fn from_config(cfg: &config::Config) -> Self {
        ...
        for adapter_cfg in &cfg.adapters {
            ...
            match platform::new_adapter_from_config(adapter_cfg) {
                Ok(adapter) => {
                    app.adapters.register(&adapter_cfg.name, adapter);
                    tracing::info!(name = adapter_cfg.name, "适配器创建成功");
                }
                Err(e) => tracing::error!("创建适配器失败：{}", e),
            }
            // match adapter_cfg.platform.as_str() {
            //     "qq" => match QQAdapter::new(&adapter_cfg.config) {
            //         Ok(adapter) => {
            ...
            //             );
            //             tracing::info!(name = adapter_cfg.name, "适配器id冲突");
            //         }
            //         Err(e) => {
